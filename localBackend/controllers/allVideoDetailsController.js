...
const connection = require('../config/localdb')

allVideoDetailsController.getAllVideoDetails = async  (req,res) =>{     
try{
     const finalResults = {};
     const getMovieQuery = `SELECT * FROM movieLocalTable`;
...
FROM advertisementLocalTable m 
INNER JOIN questionLocalTable q ON m.questionTableID = q.id`;
